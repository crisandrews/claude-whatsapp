...
- `scope.ts` module with pure `resolveScope` / `scopedAllowedChats` / `assertReadableScope` helpers (owner, non-owner defaults, per-chat overrides, terminal bootstrap, TTL fail-closed).
...
- Test files (`lib.test.ts`, `db.test.ts`, `scope.test.ts`) are no longer published in the repo (now `.gitignore`d as `*.test.ts`). Tests stay dev-local; the `npm test` script was removed from `package.json`. Reduces clone size for end users.
